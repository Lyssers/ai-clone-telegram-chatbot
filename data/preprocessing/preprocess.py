...
def txt_to_csv(txt_path, csv_writer, parent_id):
  ...
  with open(txt_path, "r", encoding="utf-8", newline='\n') as txt_file:
    ...
    for i, line in enumerate(txt_file):
      ...
      if (line[0].isdigit and i == 0) or "omitted" in line or "live location" in line:
        continue

      # If the line starts with a digit, it is a new message
      if line[0].isdigit():
        print ("Line Starts with a digit!")
      ...
      # If the line does not start with a digit, it is a continuation of the previous message
        ...
        print ("Line does not start with a fucking digit!")
        if (message):
          print(message["text"])
          message["text"] += "\n" + line
        else:
